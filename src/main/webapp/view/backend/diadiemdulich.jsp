                                ...
                                <c:forEach var="Diadiemdulich" items="${Diadiemdulich}">
                                     <tr>
                                       <td class="text-center align-middle">${Diadiemdulich.getDddl_id()}</td>
                                            <td class="text-center align-middle">${Diadiemdulich.getDddl_ten()}</td>
                                             <td class="text-center align-middle">${Diadiemdulich.getDddl_mota()}</td>
                                            <td class="text-center align-middle">        
												<img src="${pageContext.request.contextPath}${Diadiemdulich.getDddl_path()}" class="img-fluid" width="50px" height=50px/>
                                            </td>                                     
                                            <td class="text-center align-middle">${Diadiemdulich.vitri.getVtkinhdo()}</td>
                                            <td class="text-center align-middle">${Diadiemdulich.vitri.getVtvido()}</td>
                                            <td class="text-center align-middle">${Diadiemdulich.mien.getTenmien()}</td>
                                            <td class="action"><a href="/xem/${Diadiemdulich.vitri.getVitriid()}"><button class="btn btn-secondary button1">Xem chi tiết</button></a><br/>
                                           	<a href="/home/view/${Diadiemdulich.getDddl_id()}"><button class="btn btn-info button1">Update</button></a><br/>
                                           	<form:form 
											        method="POST"
